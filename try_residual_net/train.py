...
from try_residual_net.data_set import load_data
...
def main():
    ...
    results = model.fit_generator(train_data.flow(x_train, y_train, batch_size=batch_size),
                                  ...
                                  epochs=epochs,
                                  callbacks=[LearningRateScheduler(partial(getitem, tuple(take(epochs, concat(repeat(0.1, 80), repeat(0.01, 42), repeat(0.01))))))],
